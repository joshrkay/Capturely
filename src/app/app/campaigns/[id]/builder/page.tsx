...
function CampaignSettingsPanel({
  ...
  const trigger = campaign.triggerJson ? JSON.parse(campaign.triggerJson) : { type: "immediate" };
  const targeting = campaign.targetingJson ? JSON.parse(campaign.targetingJson) : { type: "all" };
  const frequency = campaign.frequencyJson ? JSON.parse(campaign.frequencyJson) : {};

  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Campaign Settings</h3>

      {/* Targeting */}
      <div>
        <label className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">URL Targeting</label>
        <select
          value={targeting.type}
          onChange={(e) => onUpdate({ targetingJson: JSON.stringify({ ...targeting, type: e.target.value }) })}
          className="w-full rounded border border-zinc-300 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
        >
        ...
        {targeting.type !== "all" && (
          <input
            ...
            onChange={(e) => onUpdate({ targetingJson: JSON.stringify({ ...targeting, value: e.target.value }) })}
            placeholder="URL pattern"
            className="mt-1 w-full rounded border border-zinc-300 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
          />
        )}
      </div>
      ...
      {/* Trigger */}
      <div>
        <label className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Trigger</label>
        <select
          value={trigger.type}
          onChange={(e) => onUpdate({ triggerJson: JSON.stringify({ type: e.target.value }) })}
          className="w-full rounded border border-zinc-300 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
        >
        ...
        {trigger.type === "delay" && (
          <input
            ...
            onChange={(e) => onUpdate({ triggerJson: JSON.stringify({ type: "delay", delayMs: parseInt(e.target.value) }) })}
        ...
        {trigger.type === "scroll" && (
          <input
            ...
            onChange={(e) => onUpdate({ triggerJson: JSON.stringify({ type: "scroll", scrollPercent: parseInt(e.target.value) }) })}
            placeholder="Scroll %"
            className="mt-1 w-full rounded border border-zinc-300 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
          />
        )}
        {trigger.type === "click" && (
          <input
            ...
            onChange={(e) => onUpdate({ triggerJson: JSON.stringify({ type: "click", clickSelector: e.target.value }) })}
      ...
      <div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <label className="text-xs text-zinc-500 dark:text-zinc-400 w-24">Max shows</label>
            <input
              type="number"
              value={frequency.maxShows ?? ""}
              onChange={(e) => onUpdate({ frequencyJson: JSON.stringify({ ...frequency, maxShows: parseInt(e.target.value) || undefined }) })}
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs text-zinc-500 dark:text-zinc-400 w-24">Every N days</label>
            <input
              type="number"
              value={frequency.everyDays ?? ""}
              onChange={(e) => onUpdate({ frequencyJson: JSON.stringify({ ...frequency, everyDays: parseInt(e.target.value) || undefined }) })}
              placeholder="unlimited"
              className="flex-1 rounded border border-zinc-300 px-2 py-1 text-xs dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
            />
          </div>
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => onUpdate({ frequencyJson: JSON.stringify({ ...frequency, perSession: e.target.checked }) })}
              className="rounded"
            />
            <label className="text-xs text-zinc-500 dark:text-zinc-400">Once per session</label>
          </div>
        </div>
      </div>
    </div>
